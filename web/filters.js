...
'use strict';
  ...
  on('dim-base-qty', 'click', function() {
    ...
    if (url.searchParams.get('is_dim') === '1') url.searchParams.delete('is_dim');
    else url.searchParams.set('is_dim', '1');
  ...
  function syncModeBtn(btnId, show, key, url) {
    ...
    setModeBtn(btn, isOr, isOr ? window._localeUI.filter[btnId === 'dim-mode-toggle' ? 'dim_mode_or' : 'qty_mode_or']
                               : window._localeUI.filter[btnId === 'dim-mode-toggle' ? 'dim_mode_and' : 'qty_mode_and']);
  }
  ...
  function syncFilterStates() {
    ...
    var vals = dimVals();
    var dimSet = vals.filter(dimResolved).length;
    var qtyN = (window._qtySelected || []).length;

    syncModeBtn('dim-mode-toggle', dimSet >= 2, 'dim', url);
    syncModeBtn('qty-mode-toggle', isFormulasView() && qtyN >= 2, 'fml', url);
    if ($('qty-mode-toggle') && !isFormulasView()) $('qty-mode-toggle').classList.add('hidden');

    toggleCls('dim-base-qty', 'active', url.searchParams.get('is_dim') === '1');
    toggleCls('dim-base-qty', 'disabled', isFormulasView());

    var allFilled = vals.every(dimResolved), hasZero = vals.indexOf('0') !== -1;
    toggleCls('dim-fill-zeros', 'active', allFilled && hasZero);
    toggleCls('dim-fill-zeros', 'disabled', allFilled && !hasZero);
    toggleCls('dim-reset', 'disabled', !vals.some(dimResolved));
  ...
  function cleanModeSwitched(url) {
    ...
    var changed = false;
    function dropIf(key, ok) {
      var i = parts.indexOf(key);
      if (!ok && i !== -1) { parts.splice(i, 1); changed = true; }
    }
    ...
    dropIf('dim', dimCount >= 2);
    var qp = url.searchParams.get('qty');
    dropIf('fml', qp && qp.split(',').length >= 2);
    if (changed) {
  ...
  (function() {
    ...
    function chevronSvg() {
      ...
      ['width', 'height'].forEach(function(a) { svg.setAttribute(a, '14'); });
    ...
    function initCSelect(select) {
      ...
      function render() {
        ...
        $$('option', select).forEach(function(option, i) {
          if (i === sel) return; /* shown in the trigger, not the list */
          ...
          sp.textContent = option.textContent.trim();
          btn.appendChild(sp);
  ...
  function buildFilterUrl() {
    ...
    if (_topicTree && _topicTreeMode === 'checkbox') {
      ...
      if (allRootNodesChecked()) sp.delete('ids');
      else sp.set('ids', ids.join(','));
    } else {
      sp.set('ids', '');
    }

    var dMin = Math.round(parseFloat($('diff-min').value)), dMax = Math.round(parseFloat($('diff-max').value));
    ...
    if (url.pathname !== '/formulas' && url.pathname !== '/quantities' && url.pathname !== '/search') {
  ...
  /* Both inputs have pointer-events:none so the track catches the click;
     dispatch onto the nearer thumb (native behavior would route
     everything to the topmost input). */
  (function() {
