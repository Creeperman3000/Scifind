...
class UnitGraph:
    ...
    def __init__(self, conn, quantity_id):
        ...
        for row in conn.execute(
            "SELECT id, symbol, system, is_base, reference_unit_id, reference_expr "
        ...
        for row in conn.execute(
            "SELECT id, unit, symbol_overwrite, system, is_base "
    ...
    def _ensure_unit_loaded(self, unit_id):
        ...
        (and not a compound row). Returns the edge (ref, expr) or None
        if the unit doesn't exist or is compound."""
        ...
        row = self.conn.execute(
            "SELECT id, reference_unit_id, reference_expr "
            "FROM unit WHERE id = ?",
            (unit_id,),
        ).fetchone()
        if row is None:
    ...
    def path_to_root(self, unit_id):
        ...
        The root may be a unit row (reference_unit_id IS NULL) OR a
        compound_unit row (which acts as the SI base for its quantity).
        """
        seen = set()
        path = []
        cur = unit_id
        while cur is not None:
            ...
            seen.add(cur)
            path.append(cur)
            # Compound unit = root, stop walking.
            if cur in self.compound_rows:
                break
    ...
    def root_value(self, unit_id):
        ...
        The root is the last node of `path_to_root`. If the path ends
        in a unit row, that's the SI base. If it ends in a compound
        row, that's the SI base too — the chain's product is the value
        in compound-unit terms, but compound-unit dimensions differ
        from the unit's, so the caller should use `root_value_compound`
        for paths ending at a compound.
        """
        path = self.path_to_root(unit_id)
        if path is None:
            return None
        # If the path ends at a compound, return None — the caller
        # should use root_value_compound instead.
    ...
    def root_value_compound(self, compound_id):
        """Composite unit's root value: product of (unit^exp) over its parts."""
        row = self.compound_rows.get(compound_id)
        if row is None:
            return None
        try:
    ...
    while stack:
        ...
        for other_id, (other_ref, _) in graph.edges.items():
            if other_ref == cur and other_id != cur and other_id not in visited:
                stack.append((other_id, path + [other_id]))
    return None
...
def render_equation(graph, row_id, ref_id):
    ...
    For multiplicative chains that touch a constant or involve offsets,
    emits a symbolic equation form ('x ref = ... x row') so the constant
    or offset term stays visible. For pure-numeric multiplicative chains,
    emits a compact numeric form ('1 row = N ref').

    Walks the graph in either direction: forward (row→root→...→ref) or
    backward (ref→...→row→root), picking whichever path exists and
    handling each direction's inversion correctly.
    """
    if row_id == ref_id:
        return None
    row_sym = _unit_latex(graph, row_id)
    ...
    # Try forward (row → ref, possibly via intermediate roots).
    path_fwd = path_between(graph, row_id, ref_id)
    if path_fwd is not None and len(path_fwd) >= 2:
        return _emit_path_equation(graph, path_fwd, row_sym, ref_sym)

    # Try inverse (ref → row, possibly via intermediate roots). The chain
    # `path_inv` has the same edges in the opposite direction; we invert.
    path_inv = path_between(graph, ref_id, row_id)
    if path_inv is not None and len(path_inv) >= 2:
        return _emit_inverse_path_equation(graph, path_inv, row_id, ref_id, row_sym, ref_sym)

    return None


def _emit_path_equation(graph, path, row_sym, ref_sym):
    """Emit the equation for a forward-direction path.

    path = [row_id, ..., ref_id].
    """
    # Detect constants/offsets — only over edges that exist
    # (intermediate nodes).
    has_constant = False
    has_offset = False
    x = 1.0
    for cur in path[:-1]:
        ...
        if not math.isfinite(x):
            return None
    if has_offset:
        return _render_offset_equation(graph, path, row_sym, ref_sym, graph.conn)
    if has_constant:
...
def _emit_inverse_path_equation(graph, path_inv, row_id, ref_id, row_sym, ref_sym):
    """Emit the equation for a backward-direction path.

    path_inv = [ref_id, ..., row_id] (ref points forward toward row).
    We compose edges forward along path_inv, then invert the result.
    """
    # Detect constants/offsets — but only over edges that exist
    # (the last element has no outgoing edge).
    has_constant = False
    has_offset = False
    for cur in path_inv[:-1]:
        expr = graph.edges.get(cur, (None, None))[1]
        if expr is None:
            return None
        for step in (expr if isinstance(expr, list) else [expr]):
            ...
            if step.get("op") in ("add", "sub"):
                has_offset = True
    if has_offset:
        # Multi-edge offset chain; we currently can't render the
        # inverse direction symbolically — fall back to numeric.
        return _render_numeric_equation(graph, row_id, ref_id)
    if has_constant:
        # Use the symbolic factor path with row/ref swapped.
        return _render_symbolic_factor(graph, path_inv, ref_sym, row_sym)
    # Pure multiplicative: compute the ratio.
    x = 1.0
    for cur in path_inv[:-1]:
        expr = graph.edges[cur][1]
        x = _apply(expr, x, graph.constants_dict())
        if not math.isfinite(x):
            return None
    factor_str = _pretty_factor(1.0 / x)
    if factor_str == "1":
        return f"1\\,{row_sym} = 1\\,{ref_sym}"
    return f"1\\,{row_sym} = {factor_str}\\,{ref_sym}"
